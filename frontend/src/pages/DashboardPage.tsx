        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', color: '#94a3b8', fontSize: '0.88rem' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#60a5fa', fontWeight: 600 }}>
              <GraduationCap size={15} /> {profile?.degree || 'B.Tech'} {profile?.major_or_branch || 'CSE'} — Year {profile?.academic_year || 3}
            </span>
            <span>•</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Target size={14} color="#34d399" /> Target Role: <strong style={{ color: '#f8fafc' }}>{profile?.target_career_title || 'Software Engineer'}</strong>
            </span>
          </div>
        </div>
                ...
                {learningPath.milestones.slice(0, 4).map((m: any) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      >
                        {m.status === 'COMPLETED' ? '✓' : m.step_number}
                      </div>
                      <div>
