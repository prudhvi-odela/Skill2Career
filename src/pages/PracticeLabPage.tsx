                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '6px' }}>
                      {executionResult.test_results?.map((tr: any, idx: number) => (
                        <div key={idx} style={{ fontSize: '0.75rem', color: tr.passed ? '#15803d' : '#b91c1c', fontFamily: 'monospace' }}>
                          {tr.passed ? '[PASS]' : '[FAIL]'} Test Case {idx + 1}: {tr.passed ? 'Passed' : `Expected ${tr.expected_output}, got ${tr.actual_output || 'Error'}`}
                        </div>
                      ))}
                    </div>
                  </div>
