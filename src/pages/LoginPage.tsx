...
export const LoginPage: React.FC = () => {
  ...
  // Handle live mouse move for dynamic spotlight, 3D tilt, and parallax
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmailFormat(email)) {
      setError('Please provide a valid student email address (e.g. name@university.edu).');
      return;
    }

    if (!password) {
      setError('Please enter your student account password.');
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.detail ||
          'Authentication failed. Please verify your student email and password or register.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      {/* ── Floating Parallax Student Diagnostic Badges ─────────────────── */}
      <div className="hidden lg:block pointer-events-none absolute inset-0 z-10 max-w-6xl mx-auto">
        ...
        >
          ...
          <div>
            ...
            <div className="text-[11px] text-slate-500">Real-time student diagnostic</div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <div className="font-semibold text-slate-900 text-[13px]">94% Match Index</div>
            <div className="text-[11px] text-emerald-600 font-medium">Tier-1 drives open</div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <div className="font-semibold text-slate-900 text-[13px]">Placement Hub</div>
            <div className="text-[11px] text-slate-500">Batch 2026 Campus Drive Active</div>
          </div>
        </div>
      </div>

      {/* ── Student Sign In Card (Matching Signup / Signin CSS) ───────────── */}
      <div className="z-20 w-full max-w-[460px]">
        ...
        >
          {/* Specular Glare reacting to mouse */}
          <div
          ...
          {/* Official Student OAuth Buttons Section */}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label className="input-label">Student University Email</label>
              <input
                type="email"
                required
